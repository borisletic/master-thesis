...
The own dataset (main contribution). Two files of JSON Lines:
...
Each benign prompt is paired (via `pair_id`) with a harmful contrast that shares
trigger vocabulary but differs in intent — isolating *lexical* over-refusal from
*contextual* judgment (RQ4).
...
| `pair_id`        | string    | yes      | links benign↔harmful contrast (`pair-001`)                            |
